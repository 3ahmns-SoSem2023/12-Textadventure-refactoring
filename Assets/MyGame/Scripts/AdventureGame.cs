...
public class AdventureGame : MonoBehaviour
{
    ...
    private const int DEHYDRATIONCOUNT = 20;
    ...
    private State doTransition(State currentState, State nextState)
    {
        ...
        passedStatesCount += 1;
        dehydrationCount = (dehydrationCount < DEHYDRATIONCOUNT) ? dehydrationCount += 0.5 : dehydrationCount = DEHYDRATIONCOUNT;
        ...
        if (dehydrationCount == DEHYDRATIONCOUNT)
        {
        ...
        if (currentState.name == nextState.name)
        {
            if (nextState.name == STATEKNITDO || nextState.name == STATEFIGHTATTACK || nextState.name == STATECOLLECTDO)
            {
                wait = false;
        ...
        if (currentState.name == STATEFIGHTDO && (nextState.name == STATECOLLECTINFO || nextState.name == STATEFIGHTDO))
        {
